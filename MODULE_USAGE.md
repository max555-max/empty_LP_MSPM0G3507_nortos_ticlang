...
当前 `main()` 是任务选择入口，初始化顺序为：

1. `SYSCFG_DL_init()`
2. OLED 显示任务选择界面，PB8 切换、PA7 启动
3. 按 PA7 后进入 Task2、Task3、Task4 或 Task5 的原有无限循环

当前主循环顺序为：

1. 读取 PB8 / PA7 两个低有效按键
2. PB8 在 Task2～Task6 之间循环切换
3. PA7 启动当前任务；Task6 当前为保留项，不会启动执行器
...
PB8/PA7 任务选择 -> 已选任务的独立数据链路。选择界面不运行 Task2～Task5 的控制循环，并将两路驱动电机 PWM 置零。
